...
<body>
            <?php  
                ...
                $in_list ="";
                $dept_id= $con->query("SELECT Department_id,Department_name FROM department");
                 while($row = $dept_id->fetch_row()){
                    $in_list.="<option value = ".$row[0].">".$row[1]."</option>"; 
                 }
                 $in_list_2="";
                 $head_emp= $con->query("SELECT Employee_id FROM employee");
                 while($row = $head_emp->fetch_row()){
                    $in_list_2.="<option value = '".$row[0]."'></option>"; 
                 }
                 $data ="";
                 $Dept_profile = $con->query("SELECT department.Department_manager_id,employee.Name,department.Department_name
                 FROM employee
                 inner join department on employee.Employee_id = department.Department_manager_id 
                 ORDER BY Department_id 
                 ");  
                    while($row = $Dept_profile->fetch_row()){
                    $data .= "<tr>".
                    "<td>".$row["0"]."</td>".
                    "<td>".$row["1"]."</td>".
                    "<td>".$row["2"]."</td>".
            ...
            <div class=\"wrapper\">
                      ...
                      <form class=\"data-form\" action=\"edit_dept_imp.php\" method=\"POST\" >   
                      <span class=\"fas fa-wrench\"> </span> 
                      <h1>แก้ตำแหน่งหัวหน้าแผนกงาน</h1>
                     <p>
                      หมายเลขทะเบียนแผนกงานงาน <input list =\"dept_id\" placeholder=\"  หมายเลขทะเบียนแผนกงานงาน \" name=\"dept_id\" required autocomplete =off>
                       <datalist id =\"dept_id\" >
                       ".
                       $in_list
                       ."
                       </datalist>
                       เลขทะเบียนพนักงาน <input list =\"emp_id\" placeholder=\" เลขทะเบียนพนักงาน\" name=\"emp_id\" required autocomplete =off>
                       <datalist id =\"emp_id\" >
                       ".
                       $in_list_2   
                       ."
